...
singleSampleSeg<- function(mSetsAnno, thresh, colour.amplification, colour.loss, array_type, showPlot){
  ...
  foreach::foreach(j = 1:nrow(segmentation_data2)) %do% {
    if (segmentation_data2$segmean_OLD[j] > (segmentation_data2$noise[j] * 0.25)) {
      segmentation_data2$segmean[j] <- 1
    } else if (segmentation_data2$segmean_OLD[j] < -(segmentation_data2$noise[j] *0.25)) {
      segmentation_data2$segmean[j] <- -1
    } else {
  ...
  return(list(segmentation_data, toChange, frac, strength))
}
